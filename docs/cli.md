...
An open issue is eligible for label-only autonomous execution when it has `agent-ready`, exactly one configured priority label, and `autonomous-merge`, all backed by immutable events from currently authorized non-App actors. No `/agent-symphony approve` comment is required in that mode. Human-review issues still require the exact approval comment. Any required-label or body change invalidates the saved authorization; after a body edit, remove and reapply `autonomous-merge` to authorize the new revision.
