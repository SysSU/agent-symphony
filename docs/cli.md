...
- `control` asks the already-running daemon to execute its existing guarded dashboard action. It derives a private local socket from the exact `--runtime-state`, requires that socket and state root to belong to the caller, verifies `--repository` against `deployment.json`, and never reads or transmits the dashboard password. Each request has a bounded `--timeout` (30 seconds by default, two minutes maximum) and a non-secret `--request-id` (randomly generated when omitted); `--json` returns the action's versioned result, underlying status, and whether a busy result is retryable. Supply an explicit ID when a timeout may need retrying, then reuse that same ID and exact input. Before mutation, the daemon durably records the ID bound to the complete request. Retrying the same ID and input returns the first completed result without replaying the action, including after restart. Reusing an ID with different input is refused. A crash with an unknown outcome leaves a pending receipt that refuses replay. The private receipt file retains at most 128 entries, evicts the oldest completed entry when needed, and never evicts an unknown pending outcome. Reconcile runs inside the daemon's operation mutex instead of competing for `daemon.lock`. Dismissal, archive, abandon, and permanent removal interrupt an active reconciliation, take exclusive ownership, and finish their server-side state and cleanup before returning. Other controls retain the existing busy response. All controls pass through the same validation and mutation handler as the browser. Archive, abandon, and removal require `--confirm`. A successful command proves that action returned successfully; re-read status or GitHub before claiming a separate downstream workflow changed.
- `serve --state path --runtime-state path` verifies the authenticated GitHub CLI account and repository, binds the runtime-state root to that repository in `deployment.json`, acquires a non-following single-instance lock, reconciles immediately, and polls on the configured stable cadence across bounded GitHub failures. Identical reads are reused only within that cycle; any mutation invalidates them. Each cycle logs JSON timing, GitHub request and retry counts, success, and the failing phase. A later command that names another configured repository for that root fails before reading project runtime state. `--interval` overrides the repository setting for that run. It also creates the private running-daemon control socket and serves that repository's dashboard at `--dashboard-address` (default `127.0.0.1:8080`). Repeat `--dashboard-project URL` to present another deployment's status read-only; the URL must be an HTTP(S) origin without credentials, path, query, or fragment. Peer controls remain on the peer dashboard and are never proxied. Localhost or a loopback IP is required unless `--allow-unsafe-dashboard-network` is set; that opt-in requires `--dashboard-password-file`. The file must be a coordinator-owned regular file with no group or other permissions and one nonempty password line. The coordinator reads it without placing the password in process arguments. HTTP Basic authentication protects every dashboard route using username `agent-symphony`; a password file may also protect loopback without the unsafe flag. Every cycle has a whole-cycle five-minute deadline. Use `control --action reconcile` to reconcile the running daemon; the older `reconcile` command remains a standalone production cycle for stopped deployments. CLI controls retry a busy daemon only until `--timeout`; dashboard controls stop after ten busy responses and retain the server's retry guidance. `status`, `list`, and `inspect` refresh and expose the same queued, active, blocked, review-ready, and completed projection. Supplying `--attempts path` selects the nonmutating offline diagnostic. No GitHub credential or identity environment variables are required or read. Independent repository daemons on one host must use distinct `--state`, `--runtime-state`, and dashboard addresses.
...
Every successful refresh calculates a **status projection**: the current view built from GitHub and local runtime facts. It rejects a status belonging to any repository other than the bound deployment project, then atomically writes the view to `<runtime-state>/status.json` with mode `0600`. The file includes a timestamp, issue state, current phase, blockers, diagnostics, next action, and a bounded list of attempt-owned sessions. Each session records its role, deterministic name, lifecycle state, current marker, and available timestamps. Reviewer entries also record `plan-review` or `implementation-review` mode and the exact target. A failed daemon refresh retains the last successful timestamp and statuses and adds a redacted `reconciliation_error` with its failure timestamp; the next successful refresh clears both fields. The same status fields appear in `status` and `inspect`. This is the latest snapshot, not an append-only history.
...
The dashboard reads that snapshot every five seconds. When peer URLs are configured, its project selector reads each peer's repository-bound status and hidden-card state but exposes peer cards read-only. A peer that returns another repository's status is rejected; open the peer dashboard link to use that deployment's controls. Its five-lane board—Queue, In progress, In review, Needs attention, and Done—shows current and completed attempts at the same time. When a newer attempt exists for the same issue, older failed, orphaned, or cancelled attempts move to a collapsed Previous attempts section. Their diagnostics remain visible, but they do not affect current state counts or health. For a terminal attempt whose issue is closed, **Dismiss (keep diagnostics)** hides only that exact current or previous-attempt card after the server rechecks the projection and GitHub issue. The manifest, log, worktree, issue, and pull request remain unchanged, and the dismissal survives refresh and restart in `<runtime-state>/dashboard-state.json`. **Permanently remove** is available only for a non-current terminal entry. It verifies the projected identity and exact manifest, records a private durable intent, rejects dirty or unpublished work, then removes only that attempt's deterministic implementation/review sessions, worktree, snapshots, results, manifest, logs, and handoff artifacts. The `removed` dashboard marker keeps the card absent after reload or reconciliation; an interrupted cleanup resumes from `<runtime-state>/removal-state.json`. The GitHub issue, pull request, and remote refs are unchanged. Archive is available only for a completed projection and reuses completed-attempt cleanup before adding a hidden-card marker. Abandon is available only for an orphaned projection; it validates and removes the exact implementation and reviewer sessions, worktree, result, snapshots, and retained manifest/log, then hides the stale card. These four terminal actions preempt reconciliation and return only after their durable server-side work finishes. Recover revalidates one exact stuck attempt, may mark it failed, and posts the fixed retry control. Destructive controls require browser confirmation; all controls require a same-origin POST and the checks described below. The browser supplies the deployment repository plus issue/attempt numbers; paths, branches, sessions, commands, and arbitrary GitHub policy are never accepted from it. See [Recovery](recovery.md).
