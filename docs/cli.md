...
Attempt cards also list the bounded implementation and reviewer sessions retained by the manifest. The primary implementation selector uses `/terminal`; reviewer selectors use `/reviewer/terminal`. Both routes require the exact deployment repository, issue, and attempt, then derive and compare the deterministic tmux name before attaching. Reviewer terminals are read-only, while implementation terminals keep their existing interactive behavior. Unknown roles remain visible in lifecycle data but are not attachable.
