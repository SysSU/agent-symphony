...
During an active attempt, other nonempty issue comments posted after the coordinator's active-attempt marker are implementation feedback once the attempt has a pull request. Pull-request conversation comments, inline review comments, and review bodies use the same path. The coordinator freshly requires `write`, `maintain`, or `admin` permission before each handoff, and excludes control commands and its own workflow comments. These comments and confirmed dashboard messages amend the issue contract in chronological order: a later human instruction wins over conflicting earlier text, and automated review findings never override a human instruction.
