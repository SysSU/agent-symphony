...
Run `scripts/validate-release.sh VERSION`. It confirms that the committed dashboard export and lockfile are present, then executes all tests with the race detector, the repository lint gate, a no-CGO build, focused orchestration/security checks, two byte-identical four-target builds, SHA-256 verification, a scan of every regular candidate file (including ignored and untracked files), and documentation-presence checks. It does not prove that the export matches the dashboard source; macOS and Linux CI rebuild it and reject a diff. Rebuild the export first with `scripts/build-dashboard.sh` when dashboard sources or lockfiles change. `scripts/release.sh VERSION OUTPUT` refuses to overwrite an output directory and emits four `agent-symphony_VERSION_OS_ARCH.tar.gz` archives plus `SHA256SUMS`.
