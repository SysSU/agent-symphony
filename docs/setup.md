...
- `commands.implementation` / `commands.reviewer` — the coding-agent CLI to run (defaults to `codex exec --sandbox workspace-write` for noninteractive source edits plus plain `codex exec --sandbox read-only -` for exact-change review). Agent Symphony's boundary helper delivers the prompt through an anonymous descriptor and captures stdout in an exclusively created private file outside the assigned worktree or immutable review snapshot while stderr stays in tmux. Implementations must emit one bounded `agent-symphony-result-v1` JSON object; reviewers must emit one bounded `agent-symphony-review-v1` object. A boundary-owned process-group leader pins the group identity until remaining members are terminated and capture is drained on completion, overflow, or cancellation. An output handle that remains open outside that group makes the attempt fail after a bounded drain; retained results support safe retries and never enter source commits.
