...
- `commands.implementation` / `commands.reviewer` — the coding-agent CLI to run (defaults to `codex exec --sandbox workspace-write` for noninteractive source edits plus read-only `codex review`). Agent Symphony redirects implementation stdout to a private result file outside the assigned worktree while stderr stays in tmux; default and custom implementations must therefore emit exactly one `agent-symphony-result-v1` JSON object of at most 64 KiB on stdout and send progress to stderr. Over-limit capture fails during execution. The retained result supports export retries and never enters the source commit.
