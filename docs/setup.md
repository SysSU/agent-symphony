...
- `commands.implementation` / `commands.reviewer` — the coding-agent CLI to run (defaults to `codex exec --sandbox workspace-write` for noninteractive source edits plus read-only `codex review`). Agent Symphony's boundary helper delivers the prompt through an anonymous descriptor and redirects implementation stdout concurrently to an exclusively created private result file outside the assigned worktree while stderr stays in tmux. Default and custom implementations must therefore emit exactly one `agent-symphony-result-v1` JSON object of at most 64 KiB on stdout and send progress to stderr. A boundary-owned process-group leader pins the group identity until remaining members are terminated and capture is drained on completion, overflow, or cancellation. An output handle that remains open outside that group makes the attempt fail after a bounded drain; the retained result supports export retries and never enters the source commit.
