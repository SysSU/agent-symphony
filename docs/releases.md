...
```sh
AGENT_SYMPHONY_FULL_SYSTEM_E2E=1 go test ./cmd/agent-symphony -run '^Test(FullSystemE2E|HistoricalAttemptActionsFullSystemE2E|DashboardLifecycleFullSystemE2E|DashboardOrchestratorFullSystemE2E)$' -count=1 -timeout 10m
AGENT_SYMPHONY_FULL_SYSTEM_E2E=1 AGENT_SYMPHONY_FULL_SYSTEM_RACE=1 go test -race ./cmd/agent-symphony -run '^Test(FullSystemE2E|HistoricalAttemptActionsFullSystemE2E|DashboardLifecycleFullSystemE2E|DashboardOrchestratorFullSystemE2E)$' -count=1 -timeout 10m
...
The live command must name the exact private sample repository in its JSON report. The preflight rejects a run ID found on any prior pilot issue, so choose a new ID for every invocation. A blocked report is a failed release gate, not permission to clean unrelated issues, pull requests, branches, worktrees, sessions, or runtime data.
