...
- **External runtime prerequisites:** Git, tmux, configured coding-agent executables, repository access, and GitHub connectivity. By default no separate provisioning step is required; `agent-symphony install-host` is an optional, one-time, host-administrator-run upgrade to the stronger isolated boundary (see "Credential isolation" below). `doctor` checks versions and, depending on which boundary is active, either the zero-admin local root or the provisioned identities/privilege rules/behavior, before `serve` accepts work. Package-manager and container distribution are post-MVP.
...
Authorization must also survive restart without trusting daemon memory. Control metadata is a canonical, sorted representation of readiness, priority, dependencies, path scope, completion/review policy, cancellation/retry intent, and the issue body sections that carry those values. A body revision is the SHA-256 of the exact body plus its latest immutable body-edit timeline event ID. If the body has never been edited, its anchor is the immutable tuple `(issue node ID, created_at, author ID)`. Issue-wide `updated_at` is never used because labels, comments, and other unrelated activity can change it. Human-review body controls are inert until an authorized actor posts the configured explicit approval command after the anchored body revision. Autonomous-merge controls instead require current authorized readiness, priority, and autonomous-label timeline provenance, with the autonomous label no older than initial issue creation and strictly newer than any body edit.

After accepting controls, the App writes a new App-authored snapshot comment binding the normalized-control SHA-256 hash, body hash and edit-event/creation anchor, and immutable timeline event IDs/actor IDs for non-body controls. Human-review snapshots also bind the immutable approval-command comment ID and actor ID; label-only autonomous snapshots leave those fields empty. The snapshot contains no independent policy value. Every provenance actor is freshly authorized. Human-review approval must remain exact and later than the bound body/control events; label-only autonomous authorization must retain the exact qualifying label set and body boundary.
...
By default, the implementation/review boundary runs as the same OS user as the coordinator: no separate account, no `install-host` step. This preserves credential-environment isolation but not OS-identity isolation. The boundary still assembles an explicit environment allowlist, strips GitHub/SSH/cloud/proxy credentials and the App key, disables the Git credential helper, and removes remotes from attempt worktrees, through the same validated command/path/environment logic the advanced mode uses below. It provisions private (mode `0700`) worktree/snapshot roots under the coordinator's own runtime state directory instead of the root-owned paths described below, and its verification hook proves that each root is writable and correctly permissioned rather than running the advanced mode's cross-UID sudo allow/deny canary matrix. What it does not provide is OS-enforced separation between the coordinator and the agent process: a compromised or malicious agent, once it has an interactive shell, has the same filesystem/process access as the coordinator, including the App private key and the coordinator's own environment. See `docs/security.md` for the full statement of that tradeoff.
...
The runtime writes a credential-free source bundle inside the provisioned attempt root, then the worker boundary creates its isolated repository/worktree there and invokes the exact implementation launch as `agent-symphony-worker`; that mode starts the named tmux session and deterministic primary command in the worktree. The prompt contains the issue contract, repository guidance, attempt identity, prior authorized feedback, allowed actions, and a required structured result. Independent review checks out the exact attested worker object in an immutable snapshot, persists its head/session state across reconciliation cycles, and requires a bounded structured clean/findings result. Findings enter the existing durable implementation handoff/rework lifecycle; only clean review permits publication. Process control uses process groups: graceful interrupt, bounded wait, then targeted kill of only that attempt. Attempts are retained for open PRs/rework and removed only after merge/abandonment is durable in GitHub.
...
- **Issue body edits could silently change execution controls:** the App snapshot binds body/control hashes and the latest body-edit event or initial creation anchor. Human review requires a later explicit approval; autonomous merge requires a current authorized autonomous-label event at the creation boundary or after the edit, so unrelated activity is harmless but a later body edit blocks.
- **A restart could forget who authorized current controls:** current normalized metadata/revision must match an App-authored snapshot bound to either the still-valid approval command or current label-only autonomous provenance; ambiguity blocks.
