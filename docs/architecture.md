...
The design follows the useful boundaries in the [OpenAI Symphony specification](https://github.com/openai/symphony/blob/main/SPEC.md): a single scheduling authority, a tracker adapter, deterministic workspaces, an agent runner, and an operator status surface. It deliberately differs in three places required by this product: GitHub owns the whole delivery lifecycle, portfolio policy is coordinator code rather than agent prompt logic, and authorized agents share the installed GitHub CLI capability. The upstream Elixir implementation is a prototype; its in-memory blocked state and runtime dependency make it a reference, not the release base.
...
- **One `agent-symphony` executable.** The same binary provides `install-host`, `agent-host`, `init`, `validate`, `config view`, `serve`, `control`, `chat`, `status`, `list`, `inspect`, `reconcile`, `doctor`, `diagnostics`, and `pr-governance`. `serve` owns the periodic loop, its private local CLI control socket, and its loopback-by-default dashboard; `control` invokes that running daemon's shared guarded actions without competing for its lifetime lock. Production status and standalone one-shot commands refresh from GitHub, while `--attempts` remains an offline diagnostic. Build and release scripts generate the ignored Next.js export and embed it with the xterm.js assets, so released binaries need no Node.js runtime, database, separate browser service, or container.
...
| Implementation/review agent | Edit files, run validation, assess/update documentation, use the shared GitHub CLI for authorized comments, labels, links, and direct status work, return structured outcome | Policy decisions, Git push, PR publication, or merge |

An optional repository orchestrator agent is a long-lived advisory operator console, not another scheduler. Its deterministic tmux session is `as-o-<repository-id>`. In zero-admin mode the pane runs as the coordinator user; advanced host isolation launches it through the reviewer identity boundary. A projection change starts or coalesces `commands.orchestrator_audit` as a separate one-shot process; unchanged nonterminal work also starts one no more than once every five minutes. The audit gets the current projection, previous audit time and bounded completed report, any bounded reconciliation diagnostic, and instructions for tracing the expected workflow transition. A workflow or GitHub transition, commit change, meaningful tmux output change, or direct owner reply is progress. A stall is actionable only after two consecutive observations at least one heartbeat interval apart show none of those signals. Its prompt budgets eight live checks, 20 seconds per command, and three minutes for investigation; the host enforces a four-minute child-process deadline and bounded output through the same fixed orchestrator boundary. The audit never writes into or wakes the primary conversation. Its only mutation authority is the direct GitHub status contract: it may set one specific needs-attention reason for a verified blocker or actionable stall, skip an identical current update, and clear only a prior monitoring reason disproved by fresh evidence.
...
The configured agent sandboxes determine optional inspection capability. A full-access Codex orchestrator can query the authenticated GitHub CLI exposed through its role environment; a same-user process can also read the coordinator's tmux server, while the reviewer identity cannot. Both orchestrator prompts permit only read-only inspection and require verified, inferred, or unknown labels for material operational claims. The primary orchestrator instructions also require the complete diagnostic and retry loop on the first request to investigate a stuck attempt. Proposals pass through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and writes it to a pre-created, size-bounded proposal artifact under an exclusive file lock. Its `submitted` response proves that durable write, not coordinator acceptance. The separate fixed `orchestrator-proposal-status` adapter compares the same exact object with a coordinator-authored binding observation and cannot mutate coordinator state.
...
Direct agent status is an overlay on that authoritative lifecycle. The newest valid, unedited `needs-attention` or `clear` comment from the authenticated Agent Symphony account must agree with the issue's fixed `needs-attention` label. A missing reason or partial comment/label update remains attention rather than reporting success. Needs-attention blocks dispatch and moves the current card to the dashboard attention lane while retaining the underlying active, review, failure, or completion state. The reason remains visible as a blocker; no local status write can create or clear it.
...
1. Validate configuration and host path preconditions, then authenticate GitHub and verify access to the configured repository without reading or creating project runtime state.
...
3. Strictly load the v2 ledger or import legacy manifests, hidden-card state, removal intents, completed control receipts, and PR recovery once. Refuse ambiguous pending legacy work or an active legacy reviewer that lacks exact restart proof.
...
5. Sweep pending typed effects. Consume an exact immutable completion marker first. Retry unmarked work only after reconstructing the same sanitized request and digest from a fresh observation; otherwise retain it with a diagnostic.
...
8. On shutdown, stop admission and cancel the daemon lifecycle before joining proposal, reconciliation, operator, effect, status, supervisor, and owner workers in that order. The owner remains alive long enough to retain any incomplete durable intent.
...
The daemon's GitHub integration invokes `gh api` for reconciliation and `gh auth git-credential` for publication. It discovers the coordinator's stable user ID with `/user` and verifies the configured repository through the same authenticated session. Authorized agents may invoke the installed `gh` directly with their shared role environment. Setup fails if the daemon CLI is unavailable, unauthenticated, or lacks repository access; role commands return the same clear CLI authentication failure.
...
By default, the implementation/review boundary runs as the same OS user as the coordinator: no separate account, no `install-host` step. The boundary assembles an explicit environment allowlist that includes GitHub CLI authentication/target variables but still strips SSH, cloud, proxy, and unknown credentials. It disables the Git credential helper and removes remotes from attempt worktrees. It provisions private (mode `0700`) worktree/snapshot roots under the coordinator's own runtime state directory. This mode provides no OS-enforced separation between the coordinator and the agent process: an authorized agent can use the shared GitHub CLI and also has the same user's filesystem/process access. See `docs/security.md` for the full statement of that tradeoff.
...
Environment filtering is defense in depth, not the security boundary. The three `agent-host` modes create agent/tmux processes with explicit allowlists rather than unrestricted inherited environments. They preserve only GitHub CLI's standard authentication and target variables plus configured model credentials; remove SSH, cloud, proxy, unknown credential, helper, and key variables; disable Git credential helpers; and redact credential-shaped fields from output. Each tmux server runs under its assigned agent identity, never the coordinator identity. Advanced `sudo` rules permit the exact boundary commands to retain only the named GitHub variables supplied by Agent Symphony.

Only the coordinator performs authenticated Git fetch/push, PR publication, and merge. Those Git commands use `gh auth git-credential` in the coordinator process. Authorized roles may invoke `gh` for issue and pull-request reads or status work using the stored same-user login or supported GitHub CLI environment variables. Agent Symphony never writes those credentials to committed Git config, worktrees, snapshots, manifests, logs, or launch contracts. OS permissions remain the boundary for non-GitHub coordinator state—containers are not claimed as a security boundary.
...
The runtime writes a credential-free source bundle whose name includes the repository identity inside the provisioned attempt root, then the worker boundary creates its isolated repository/worktree there and invokes the exact implementation launch as `agent-symphony-worker`; that mode starts the named tmux session and deterministic primary command in the worktree. The prompt contains the issue's exact title/body, repository guidance, attempt identity, prior authorized feedback, allowed actions, and a required structured result. Independent review uses one shared reviewer role and command for `plan-review` and `implementation-review`; no specialized UI reviewer exists. Plan review binds the issue plan digest, while implementation review binds the exact attested base/head range. Both use the same immutable snapshot and deterministic issue/attempt reviewer session, persist mode and target across reconciliation, inherit the shared GitHub CLI direct-status contract, and require a target-bound structured clean/findings result. Authorized issue and pull-request feedback remains ordered; later human instructions supersede conflicting earlier text, and automated findings cannot supersede human instructions. A running or clean review is reusable only when its mode and exact target still match. Findings enter the existing durable implementation handoff/rework lifecycle with the same human instructions; only clean implementation review permits publication. Process control uses process groups: graceful interrupt, bounded wait, then targeted kill of only that attempt. Attempts are retained for open PRs/rework. After fresh GitHub facts report an exact published attempt as merged, the implementation boundary verifies its deterministic runtime identity, kills only its named tmux session, and removes its clone and worker result; the recovery manifest and diagnostic log remain retained.
...
Feedback and validation identity includes immutable feedback source/ID values and a validation generation. Authorized issue and pull-request feedback crosses the existing durable implementation handoff; later human instructions supersede conflicting earlier text, while automated review findings cannot supersede human instructions. Advanced orchestrator sudo preserves only the fixed GitHub CLI variable allowlist through command-scoped `env_keep` rules.

The deterministic Go coordinator remains required to serialize reconciliation, bind attempts to current GitHub facts, publish results, govern pull-request transitions, deliver review-findings rework, clean up exact attempt resources, and monitor heartbeat/attention state. It is not a routine conversation relay: issue dispatch and review routing are issue-bound, operator chat attaches directly to exact implementation or reviewer sessions, authorized roles use the shared GitHub CLI status contract, and no coordinator-authored operator-message queue exists.
...
7. **Shared GitHub CLI, isolated Git publication:** after current-version admin `install-host`, `doctor` checks setgid `2770` attempt ownership/inheritance, umask `0007`, exact sudo user/group/command permissions/denials, and the implementation, review, and orchestrator boundary paths. Non-GitHub coordinator secrets and state are unreachable. The fixed worker can access all attempt repositories but no snapshot; the reviewer can access no attempt and can only read coordinator-owned completed `0550`/`0440` snapshots. Every authorized role can read the bound issue through `gh`; missing or invalid authentication fails clearly. An attempted `git push` from either agent still fails because worktrees have no remote or helper.
