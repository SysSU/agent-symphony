...
| Dashboard | Loopback-by-default status projection, optional password-protected network binding, exact-session terminal attachment, and confirmed cleanup of exact completed/orphaned local resources | GitHub workflow mutation, arbitrary commands/resources, TLS termination, or shared multi-repository state |
...
| Dashboard (FR59-63) | Loopback-by-default Go server + embedded Next.js page | Embedded export/build, bounded file serving, network opt-in/password authentication, exact tmux WebSocket input/resize/disconnect, archive/abandon state and cleanup, cross-origin/wrong-state/identity-drift rejection, desktop/mobile accessibility and console checks |
