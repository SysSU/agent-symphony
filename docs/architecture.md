...
| Dashboard | Loopback-by-default status projection, optional password-protected network binding, exact-session terminal attachment, confirmed cleanup, narrowly constrained attempt recovery, and authenticated exact-attempt worker follow-up | Arbitrary GitHub mutation, arbitrary commands/resources, TLS termination, or shared multi-repository state |
| Implementation/review agent | Edit files, run validation, assess/update documentation, return structured outcome | GitHub API/tooling, credentials, policy decisions, push, PR creation, or merge |

An optional repository orchestrator agent is a long-lived advisory operator console, not another scheduler. Its deterministic tmux session is `as-o-<repository-id>`. In zero-admin mode the pane runs as the coordinator user; advanced host isolation launches it through the reviewer identity boundary. After launch, the coordinator never pastes or sends programmatic input to that conversation. A projection change starts or coalesces `commands.orchestrator_audit` as a separate one-shot process; unchanged nonterminal work also starts one no more than once every five minutes. The audit gets the current projection, previous audit time, any bounded reconciliation diagnostic, and instructions for tracing the expected workflow transition. Its prompt budgets eight live checks, 20 seconds per command, and three minutes for investigation; the host enforces a four-minute child-process deadline and bounded output through the same fixed orchestrator boundary. It never writes into or wakes the primary conversation.

Go persists the last audit start time in `orchestrator-agent.json` but does not classify health or stalled phases. The one-shot agent labels evidence as verified, inferred, or unknown and writes one bounded result; the default Codex command writes only its final message to a transient validated artifact so progress logs cannot displace or truncate the diagnosis. The coordinator removes that artifact after atomically replacing `orchestrator-heartbeat-report.json` in the primary managed workspace. The primary agent may read that untrusted report when relevant to the operator's question or its diagnosis, but the file does not wake it and it must reverify material claims against live authoritative evidence. A daemon restart adopts an exact live primary pane, marks an unfinished local audit report failed, and does not immediately repeat the audit. `orchestrator-context.md` remains reconstructible mode-`0600` context. A missing or dead primary pane starts a fresh generation after bounded backoff. `clear` uses role rules only, while `rebuild` adds the latest sanitized projection. Neither operation changes GitHub facts.

The configured agent sandboxes determine optional inspection capability. A full-access same-user Codex orchestrator can query the coordinator user's authenticated GitHub CLI and read the same-user tmux server; a read-only sandbox or reviewer-identity process cannot. Both orchestrator prompts permit only read-only inspection and require verified, inferred, or unknown labels for material operational claims. The primary orchestrator instructions also require the complete diagnostic and retry loop on the first request to investigate a stuck attempt. Proposals pass through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and writes it to a pre-created, size-bounded proposal artifact under an exclusive file lock. Its `submitted` response proves that durable write, not coordinator acceptance. The separate fixed `orchestrator-proposal-status` adapter compares the same exact object with a coordinator-authored binding observation and cannot mutate coordinator state.

A `message_attempt` proposal names the configured repository, one positive issue/attempt pair, and at most 8192 bytes of UTF-8 text; it still requires authenticated dashboard confirmation. A `retry_transition` proposal instead carries a bounded unique request ID and no message. The coordinator observes the artifact independently of dashboard polling. It starts one ten-minute guarded reconciliation only when a fresh read-only projection identifies the exact unblocked active attempt, its implementation session is completed, and its current phase is validation or publication. The proposal status moves from `pending` to `running`, then `succeeded`, `failed`, or `refused`; only `succeeded` proves that the coordinator pass returned successfully, so callers must still re-read the authoritative issue, pull request, and attempt projection.
...
Each manifest contains bounded attempt identity, deterministic resource paths and sessions, implementation/review state, diagnostics, timestamps, and log path. It is written by temp-file plus atomic rename, mode `0600`. Each refresh similarly replaces `status.json` with the timestamped current projection, including the current phase, blockers, diagnostics, next action, and known implementation/reviewer sessions. The session entries contain only deterministic manifest identities and lifecycle metadata. There is no configurable retention or automatic log rotation: exact reconciliation and dashboard actions remove only the resources documented for their lifecycle. Deleting the entire state root loses diagnostics and resumable terminal context, but not workflow truth.

`serve` exposes the embedded dashboard and those bounded JSON projections on a configurable loopback address by default. A non-loopback address requires the explicit unsafe-network flag and a password that protects every HTTP and WebSocket route with standard HTTP Basic authentication. Direct HTTP provides no transport encryption or rate limiting. Fixed same-origin implementation and reviewer WebSocket routes accept only an issue and attempt, re-derive the exact role-specific deterministic name, verify the live tmux session, and attach it through a PTY. Reviewer attachment uses tmux read-only mode and rejects input; disconnect ends only that tmux client. Same-origin POST actions accept issue/attempt numbers only. Archive requires a completed projection and strict branch/head cleanup. Abandon requires an orphaned projection and removes exact local resources after boundary verification. Recover freshly revalidates the latest retryable attempt and posts only the fixed retry control. An authenticated browser may also queue one bounded worker follow-up for an exact active or review-ready card; the server derives the repository from the projection, verifies the exact local runtime owner, and records the durable message. Fresh GitHub authority and target checks run during delivery before any worker starts. `dashboard-state.json` is presentation state, not a task database, and can hide only archived or abandoned cards.
...
Feedback and validation identity includes immutable feedback source/ID values and a validation generation. Confirmed operator messages use the same boundary: an authenticated dashboard POST binds either the exact orchestrator proposal or an exact projected card. Message admission uses its own narrow critical section, requires one verified deterministic local runtime owner, and writes the strict idempotent GitHub marker without waiting for repository-wide authority reconstruction; it does not launch a worker. The accepted marker is projected immediately. Acceptance cancels the in-progress coordinator pass, including a guarded transition retry, then a bounded target-only delivery takes the existing coordinator lock before changing runtime state. It does not wait for the next poll or unrelated pull-request governance. Delivery reads only the deterministic pull request and issue and requires either a `RecoverChecked`-verified live runtime or a `MatchesPublishedAttempt`-verified retained runtime. Base and published head SHAs, deterministic branch/worktree/session identity, and any competing active binding participate in that proof. A stale `running` manifest is monitored against the exact tmux pane before it can delay a queued message. Delivery waits while a real pane is live, rejects stale/cancelled/completed/merged targets, and starts at most one safe follow-up turn per message. Before execution it writes a durable GitHub delivery claim, then rereads that authoritative issue and attempt again at the worker acceptance boundary; terminal reconciliation wins by recording a rejection without starting the worker. A restart resumes a claimed message with its stable key. The default Codex implementation command starts a fresh noninteractive session in the exact retained worktree, avoiding an unbounded dependency on the size or health of a prior transcript; explicit custom implementation commands run unchanged. The boundary persists the command binding, starts the replacement worker in the deterministic tmux pane, and records the exact launch identity only after the worker produces observable startup output. Startup failure wakes the waiting coordinator without granting delivery. A retired handoff protocol that remains live past the recovery deadline is interrupted and its exact deterministic tmux session is recreated before a queued message starts. Recovery refuses command drift instead of launching the durable message again. Worker-writable inbox, launch, and receipt files are not publication authority; only the coordinator-authored delivered-message state permits an operator follow-up to publish. Stable content-derived keys make duplicate confirmation and restart reconciliation idempotent. Dashboard status projects internal claims as `queued` and exposes only message IDs, operator-facing outcomes, timestamps, and bounded content-free diagnostics. Terminal failures use a strict coordinator-authored issue marker, which dispatch reconciliation reads as authoritative suppression until an authorized retry creates a later attempt. Every boundary adapter launch receives only the minimal platform environment (`PATH`, temporary-directory location, and Windows system root when present), never inherited coordinator credentials.
