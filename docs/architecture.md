...
An optional repository orchestrator agent is a long-lived advisory operator console, not another scheduler. Its deterministic tmux session is `as-o-<repository-id>`. In zero-admin mode the pane runs as the coordinator user; advanced host isolation launches it through the reviewer identity boundary. The coordinator sends bounded, redacted full-projection changes to the conversation and keeps `orchestrator-agent.json` and `orchestrator-context.md` as mode-`0600`, reconstructible local state. A daemon restart adopts an exact live pane; a missing or dead pane starts a fresh generation after bounded backoff. `clear` uses role rules only, while `rebuild` adds the latest sanitized projection. Neither operation changes GitHub facts.

The configured agent sandbox determines optional inspection capability. A full-access same-user Codex orchestrator can query the coordinator user's authenticated GitHub CLI and read the same-user tmux server; a read-only sandbox or reviewer-identity process cannot. The orchestrator instructions permit only read-only inspection, require verified, inferred, or unknown labels for material operational claims, and require the complete diagnostic and retry loop on the first request to investigate a stuck attempt. Proposals pass through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and writes it to a pre-created, size-bounded proposal artifact under an exclusive file lock. Its `submitted` response proves that durable write, not coordinator acceptance. The separate fixed `orchestrator-proposal-status` adapter compares the same exact object with a coordinator-authored binding observation and cannot mutate coordinator state.
