...
The configured agent sandboxes determine optional inspection capability. A full-access same-user Codex orchestrator can query the coordinator user's authenticated GitHub CLI and read the same-user tmux server; a read-only sandbox or reviewer-identity process cannot. Both orchestrator prompts permit only read-only inspection and require verified, inferred, or unknown labels for material operational claims. The primary orchestrator's proposals pass through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and emits a framed proposal. Emission is not coordinator acknowledgement: the separate fixed `orchestrator-proposal-status` adapter compares the same exact object with a coordinator-authored binding observation and cannot mutate coordinator state.

A `message_attempt` proposal names the configured repository, one positive issue/attempt pair, and at most 8192 bytes of UTF-8 text; it still requires authenticated dashboard confirmation. A `retry_transition` proposal instead carries a bounded unique request ID and no message. The coordinator observes proposals independently of dashboard polling. It automatically accepts a retry only when a fresh authoritative projection identifies the exact unblocked active attempt, its implementation session is completed, and its current phase is validation or publication. The coordinator durably accepts the proposal before starting the existing bounded reconciliation path; acceptance does not prove the transition completed. The durable binding status distinguishes `accepted` and `refused` retries from pending and consumed message proposals.
