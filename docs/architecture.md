...
The configured agent sandbox determines optional inspection capability. A full-access same-user Codex orchestrator can query the coordinator user's authenticated GitHub CLI and read the same-user tmux server; a read-only sandbox or reviewer-identity process cannot. The orchestrator instructions permit only read-only inspection. Its implementation proposal still passes through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and emits a framed proposal for explicit dashboard confirmation. The proposal names the configured repository, one positive issue/attempt pair, and at most 8192 bytes of UTF-8 text.
