...
| Dashboard | Loopback-by-default status projection, optional password-protected network binding, exact-session terminal attachment, confirmed cleanup, and narrowly constrained attempt recovery | Arbitrary GitHub mutation, arbitrary commands/resources, TLS termination, or shared multi-repository state |
