...
The configured agent sandboxes determine optional inspection capability. A full-access same-user Codex orchestrator can query the coordinator user's authenticated GitHub CLI and read the same-user tmux server; a read-only sandbox or reviewer-identity process cannot. Both orchestrator prompts permit only read-only inspection of live GitHub, exact tmux identities, and accessible trusted runtime artifacts. The primary orchestrator's implementation proposal passes through the fixed `agent-host orchestrator-proposal` adapter, which validates one strict bounded object and emits a framed proposal. Emission is not coordinator acknowledgement: the separate fixed `orchestrator-proposal-status` adapter compares the same exact object with a coordinator-authored binding observation. Only a matching pending binding proves current dashboard exposure; the status path cannot mutate coordinator state. The proposal names the configured repository, one positive issue/attempt pair, and at most 8192 bytes of UTF-8 text.
