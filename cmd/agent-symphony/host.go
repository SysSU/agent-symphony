...
func acceptHandoff(ctx context.Context, input []byte, root string) (string, error) {
	...
	command := agentruntime.PaneExitStatusCommand("tmux", agentruntime.HandoffPromptCommand(helper, "tmux", buffer, resultPath, launchedPath, recipient, signal, request.Command))
