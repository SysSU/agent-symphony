...
test("lifecycle action commits through the real dashboard", async ({ page }) => {
  ...
  const requestAction = action === "review-plan-cancel" ? "review-plan" : action;
  ...
  if (requestAction === "review-plan") {
    ...
    const gate = await open(reviewGate, constants.O_WRONLY | constants.O_NONBLOCK);
    try {
  ...
  if (action === "recover") {
    await expect.poll(async () => {
      const status = await fetch(`${baseURL}/status.json`, { cache: "no-store" }).then((result) => result.json());
      return status.statuses?.some((entry) => entry.issue === 73 && entry.attempt === 2);
    }, { timeout: 20_000 }).toBe(true);
    await page.reload();
