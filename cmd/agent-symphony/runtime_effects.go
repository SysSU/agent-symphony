...
func (c *runtimeEffectCoordinator) acquire(ctx context.Context, request agentruntime.EffectRequest) (*activeRuntimeEffect, error) {
	...
	for {
		...
		if previous == nil {
			...
			run := &activeRuntimeEffect{generation: request.Identity.AttemptGeneration, ctx: runCtx, cancel: cancel, done: make(chan struct{})}
			c.active[key] = run
			c.mu.Unlock()
			return run, nil
		}
...
func (c *runtimeEffectCoordinator) cancelOlder(manifest agentruntime.Manifest, generation uint64) {
	...
	c.mu.Lock()
	if run := c.active[key]; run != nil && run.generation < generation {
		run.cancel()
	}
	c.mu.Unlock()
}
